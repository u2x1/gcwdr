...
runBlogPreview root port = do
 ...
 where
  showHtml path s = do
    msg <- recv s 1024
    unless (S.null msg) $ if S.take 3 msg == "GET"
      then do
        let fileName =
              let name = (toString $ S.takeWhile (/= 32) (S.drop 4 msg))
              in  path <> name <> if last name == '/' then "index.html" else ""
